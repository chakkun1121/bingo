      ...
      <section className="print:hidden">
        ...
        <nav className="m-2">
          <label>
            生成するカードの枚数
            <input
              className="border rounded p-2 m-2 text-M"
              type="number"
              min="1"
              defaultValue={cardsNumber}
              id="cardsNumberInput"
              onChange={(e) => setCardsNumber(Number(e.target.value))}
            />
            <button
              className="p-2 m-2 border rounded bg-green-200 hover:bg-green-300"
              onClick={updateCards}
            >
              カードを作成
            </button>
